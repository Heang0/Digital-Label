        ...
        <div className="fixed inset-0 z-[500] flex items-center justify-center p-4">
          ...
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="relative w-full max-w-sm bg-white dark:bg-[#1C2434] rounded-3xl shadow-2xl overflow-hidden border border-slate-100 dark:border-slate-800">
            <div className="p-8 text-center border-b border-slate-50 dark:border-slate-800">
              ...
              <h3 className="text-xl font-bold text-[#111928] dark:text-white mb-2">{t('price_override')}</h3>
              <p className="text-sm font-medium text-[#637381] dark:text-slate-400 mb-8 px-2">{t('price_override_desc')}</p>
            ...
            <div className="flex items-center gap-3 px-6 py-5 bg-slate-50/50 dark:bg-slate-900/30">
              <Button variant="ghost" onClick={() => setActiveDiscountModal(null)} className="flex-1 font-bold text-xs h-11 hover:bg-slate-100 dark:hover:bg-slate-800">{t('cancel')}</Button>
              <Button onClick={() => {
                const el = document.getElementById('discount-input') as HTMLInputElement;
                const val = parseFloat(el?.value || '0');
                if (!isNaN(val) && val > 0 && val <= 100) executeManualDiscount(val);
                else openLabelNotice(t('invalid_discount'), t('invalid_discount_desc'), "error");
              }} className="flex-1 bg-indigo-600 hover:bg-indigo-700 text-white font-bold text-xs h-11 shadow-lg shadow-indigo-500/25 border-none">{t('update_price')}</Button>
            </div>
          </motion.div>
        </div>
      )}
