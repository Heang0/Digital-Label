...
export const SupportTab = () => {
  ...
  return (
    <div className="max-w-5xl space-y-8 pb-20">
      <div>
        <h2 className="text-2xl font-bold text-[#111928] dark:text-white tracking-tight">{t('support_center')}</h2>
        <p className="text-sm font-medium text-[#637381] dark:text-slate-400 mt-1">{t('support_center_desc')}</p>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
         {[
            { title: t('tech_helpdesk'), sub: t('priority_support'), icon: Phone, action: t('call_now'), color: 'text-[#5750F1]' },
            { title: t('email_assistance'), sub: t('response_time'), icon: Mail, action: t('open_ticket'), color: 'text-emerald-500' },
            { title: t('knowledge_base'), sub: t('guides_tutorials'), icon: MessageSquare, action: t('view_docs'), color: 'text-amber-500' },
         ].map((card, i) => (
      ...
      <div className="premium-card p-8 bg-[#F8FAFC] dark:bg-[#1C2434]">
         <div className="flex items-center gap-4 mb-6">
            ...
            <h3 className="text-lg font-bold text-[#111928] dark:text-white">{t('platform_health')}</h3>
         </div>
         <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
            {[
               { label: 'IoT Gateway', status: t('operational_status'), time: '99.9% Uptime' },
               { label: 'API Infrastructure', status: t('optimal_status'), time: '42ms Latency' },
