...
export const AdminSettings = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    bio: user?.bio || 'Super Administrator of Digital Label Systems.',
  ...
  const handleSave = async () => {
    try {
      setLoading(true);
      // Simulate save or wire to real API later
      setUser({
        ...user,
        name: formData.name,
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-700 max-w-7xl mx-auto pb-10">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6 px-1">
        <div>
          <h2 className="text-2xl font-bold text-[#111928] dark:text-white tracking-tight">{t('admin_workspace') || 'Admin Workspace'}</h2>
          <div className="flex items-center gap-2 text-xs font-medium text-[#637381] dark:text-slate-400 mt-1">
             <span>{t('dashboard') || 'Dashboard'}</span>
             <span>/</span>
             <span className="text-[#5750F1]">{t('settings') || 'Settings'}</span>
          </div>
        </div>
        ...
        {activeTab === 'profile' ? (
          <>
            <div className="lg:col-span-7 space-y-8">
              <div className="bg-white dark:bg-[#24303F] rounded-[10px] border border-[#E2E8F0] dark:border-slate-800 shadow-sm overflow-hidden transition-colors">
                <div className="px-7 py-5 border-b border-[#E2E8F0] dark:border-slate-700">
                  <h3 className="text-lg font-bold text-[#111928] dark:text-white">{t('personal_info') || 'Personal Information'}</h3>
                </div>
                <div className="p-7 space-y-6">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    ...
                    <div className="space-y-2">
                      <label className="text-sm font-medium text-[#111928] dark:text-white">{t('phone_number') || 'Phone Number'}</label>
                      <div className="relative">
                  ...
                  <div className="space-y-2">
                    <label className="text-sm font-medium text-[#111928] dark:text-white">{t('bio') || 'BIO'}</label>
                    <textarea 
                      value={formData.bio}
            ...
            <div className="lg:col-span-5 space-y-8">
              <div className="bg-white dark:bg-[#24303F] rounded-[10px] border border-[#E2E8F0] dark:border-slate-800 shadow-sm overflow-hidden transition-colors">
                <div className="px-7 py-5 border-b border-[#E2E8F0] dark:border-slate-700">
                  <h3 className="text-lg font-bold text-[#111928] dark:text-white">{t('your_photo') || 'Your Photo'}</h3>
                </div>
                <div className="p-7">
                  <ProfileUpload />
          ...
          <div className="lg:col-span-12 space-y-8 max-w-3xl mx-auto w-full">
            <div className="bg-white dark:bg-[#24303F] rounded-[10px] border border-[#E2E8F0] dark:border-slate-800 shadow-sm overflow-hidden transition-colors">
              <div className="px-7 py-5 border-b border-[#E2E8F0] dark:border-slate-700 flex items-center gap-3">
                ...
                <h3 className="text-lg font-bold text-[#111928] dark:text-white">{t('security_account') || 'Security & Account'}</h3>
              </div>
              <div className="p-7 space-y-8">
                ...
                <div className="flex items-start gap-4 p-6 rounded-2xl border border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/30">
                   ...
                   <div className="flex-1">
                      <p className="text-base font-bold text-[#111928] dark:text-white">{t('change_password') || 'Change Password'}</p>
                      <p className="text-sm text-[#637381] dark:text-slate-400 mt-1">{t('last_changed_pw') || 'Last changed 3 months ago'}</p>
                      <Button 
                      ...
                      >
                        {t('update_photo') || 'Update Password'}
                      </Button>
                   </div>
                </div>
                ...
                <div className="flex items-start gap-4 p-6 rounded-2xl border border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/30">
                   ...
                   <div className="flex-1">
                      <p className="text-base font-bold text-[#111928] dark:text-white">{t('two_factor') || 'Two-Factor Authentication'}</p>
                      <p className="text-sm text-[#637381] dark:text-slate-400 mt-1">{t('secure_2fa_desc') || 'Secure your account with 2FA codes'}</p>
                      <Button className="mt-5 h-10 px-6 text-xs font-bold bg-[#10B981] hover:bg-[#0da06f] text-white">{t('enable_2fa') || 'Enable 2FA'}</Button>
                   </div>
                </div>
              </div>
            </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-300">
           <div className="bg-white dark:bg-[#1C2434] w-full max-w-md rounded-2xl shadow-2xl border border-[#E2E8F0] dark:border-slate-800 overflow-hidden scale-in duration-300">
              <div className="px-8 py-6 border-b border-[#E2E8F0] dark:border-slate-800">
                 <h3 className="text-lg font-bold text-[#111928] dark:text-white">{t('change_password') || 'Update Password'}</h3>
                 <p className="text-xs text-[#637381] dark:text-slate-400 mt-1">{t('update_password_desc') || 'Please confirm your current password to continue.'}</p>
              </div>
              <div className="p-8 space-y-5">
                 <div className="space-y-2">
                    <label className="text-xs font-bold text-[#111928] dark:text-white uppercase tracking-wider">{t('current_password') || 'Current Password'}</label>
                    <div className="relative">
                 ...
                 <div className="space-y-2">
                    <label className="text-xs font-bold text-[#111928] dark:text-white uppercase tracking-wider">{t('new_password') || 'New Password'}</label>
                    <input 
                      type="password"
                      value={passData.new}
                 ...
                 <div className="space-y-2">
                    <label className="text-xs font-bold text-[#111928] dark:text-white uppercase tracking-wider">{t('confirm_password') || 'Confirm Password'}</label>
                    <input 
                      type="password"
                 ...
                 <div className="flex gap-4 pt-4">
                    <Button variant="outline" onClick={() => setShowPassModal(false)} className="flex-1 h-11 rounded-lg">{t('cancel') || 'Cancel'}</Button>
                    <Button onClick={handleUpdatePassword} disabled={loading} className="flex-1 h-11 rounded-lg bg-[#5750F1] text-white">
                       {loading ? <Loader2 className="h-4 w-4 animate-spin" /> : (t('update') || 'Update')}
                    </Button>
                 </div>
              </div>
           </div>
